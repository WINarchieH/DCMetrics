...
namespace DC4._0Backend.Controllers
{
    public class TimeAndAttendenceController : ApiController
    {
        ...
        [AcceptVerbs("GET", "POST")]
        [HttpGet]
        public string GetAllTimeandAttendenceEntries_Elite([FromBody]TimeAndAttendence_Elite time)
        {

            Connection connection = new Connection();

            SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings[time.Site].ConnectionString);
            SqlCommand cmd = new SqlCommand("dbo.spTimeAndAttendance_Elite", sqlConnection);
            cmd.CommandType = CommandType.StoredProcedure;




            cmd.Parameters.AddWithValue("@StartDate", time.StartDate);
            cmd.Parameters.AddWithValue("@EndDate", time.EndDate);
            ...
            List<TimeAndAttendence_Elite> entryList = new List<TimeAndAttendence_Elite>();
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        TimeAndAttendence_Elite obj = new TimeAndAttendence_Elite();
                        obj.UserID = ds.Tables[0].Rows[i]["UserID"].ToString();
                        obj.FirstName = ds.Tables[0].Rows[i]["FirstName"].ToString();
                        obj.SurName = ds.Tables[0].Rows[i]["Surname"].ToString();
                        obj.ShiftCode = ds.Tables[0].Rows[i]["ShiftCode"].ToString();
                        ...
                        obj.Approved = ds.Tables[0].Rows[i]["Approved"].ToString();
                        obj.ReasonForUpdate = ds.Tables[0].Rows[i]["ReasonForUpdate"].ToString();
                        obj.TH = ds.Tables[0].Rows[i]["TH"].ToString();
                        obj.DBL = ds.Tables[0].Rows[i]["DBL"].ToString();
                        obj.UpdateBy = ds.Tables[0].Rows[i]["UpdateBy"].ToString();
                        obj.ID = ds.Tables[0].Rows[i]["ID"].ToString();
                        obj.OTException = ds.Tables[0].Rows[i]["OTException"].ToString();
                        obj.TotalHrs = ds.Tables[0].Rows[i]["TotalHrs"].ToString();
                        obj.OTAtStart = ds.Tables[0].Rows[i]["AllowOTatStart"].ToString();
                        obj.OTAtEnd = ds.Tables[0].Rows[i]["AllowOTatEnd"].ToString();
                        obj.MealAllowance = ds.Tables[0].Rows[i]["MealAllowance"].ToString();
                        entryList.Add(obj);
