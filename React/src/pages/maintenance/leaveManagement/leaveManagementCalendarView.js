...
class DCMCalendar extends React.Component {
  ...
  componentDidMount() { // page load
    ...
    api.post('/Maintenance/LeaveManagement/GetAllApprovedLeaves').then(
      res => {
        let data = res.data;
        var holidays=[];
        ...
        data = data.map(x => {
          ...
          {  'id':x.SerialID,
             'title':x.Title,
             'start':  new Date(TestdateObjToInput(dateToDateObj(x.Date)) +'T'+x.StartTime),
             'end': new Date(TestdateObjToInput(dateToDateObj(x.Date)) +'T'+x.EndTime),
             'color':x.LeaveColor
           }
