...
const TimeAttendanceGrid = () => {
    ...
    const AfternoonAllowanceHandler = (e) => { setInputName('AfternoonAllowance', booleanToOutput(e)) };
    const AddMealBreakHandler = (e) => { setInputName('ForceAddMealBreak', booleanToOutput(e)) };
    ...
    const getTable = async (startDate, endDate) => { // TODO: API Request for table
       setOpen(true);
        ...
        await api.post('/Maintenance/TimeAndAttendence/GetAllTimeandAttendenceEntries_Makita', body).then(
            res => {
                let data = res.data;
    ...
    const addHandler = () => { // TODO: Add event handler
        ...
        api.post('/Maintenance/TimeAndAttendence/AddAttendance_Makita', body).then(
            res => {
...
//backdrop
     ...
     >
         ...
         <AgGridColumn  headerName ="Afternoon Allowance"field="AfternoonAllowance"  width={100} cellEditor= "autoCompleteEditor"  cellEditorParams= {{ options: ["Y","N"]}}/>
         <AgGridColumn  headerName ="OT Exception"field="OTException" cellEditor= "autoCompleteEditor"  cellEditorParams= {{ options: ["Y","N"]}}/>
         <AgGridColumn  headerName ="Manual Add Break"field="ForceAddMealBreak"  width={100} cellEditor= "autoCompleteEditor"  cellEditorParams= {{ options: ["Y","N"]}}/>
     ...
     <Modal title={modalTitle} buttonName={modalButtonName} onSubmit={onSubmit} showModal={showModal} unrestrictWidth={true}
                        ...
                        <div className='modal-grouping--col-4'>
                            ...
                            <Toggle name='ForceAddMealBreak' label='Add Meal Break' checked={outputToBoolean(input.ForceAddMealBreak)} onChange={AddMealBreakHandler} required></Toggle>

                            <Toggle name='AfternoonAllowance' label='Afternoon Allowance' checked={outputToBoolean(input.AfternoonAllowance)} onChange={AfternoonAllowanceHandler} hidden = {modalMode === 1 ? true : null} ></Toggle>
