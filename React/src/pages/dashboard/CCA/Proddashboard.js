...
const first = new Date(new Date().setDate(lastDay)).toDateString();
        ...
        let body = new URLSearchParams({
         ...
         //Cycle Counts
               ...
               dates =       temp.map(x=>x);
                ...
                // Hiding the cc graph table if the dates array is none
                if (dates.length === 0)
                {
                  this.setState({
                   hideCCgraph: true
...
<table hidden = {this.state.hidepickgraph} >
  <tr>
    <td>
    <Grid item xs={50}>
<Paper>
...
<table hidden = {this.state.hideputgraph}>
  <tr>
    <td>
    <Grid item xs={50}>
<Paper>
