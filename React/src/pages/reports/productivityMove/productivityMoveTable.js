...
const tableTitle = 'Productivity Move Report';
...
const ProductivityPickReport = () => {
    ...
    const tableColumns = [
        ...
        { Header: 'No Of Move', accessor: 'NoOfMove', modalType: 'textbox' },
        { Header: 'No Of Units', accessor: 'NoOfUnits', modalType: 'textbox'},
        { Header: 'Moves Per hour', accessor: 'Units_Hr', modalType: 'textbox' },
    ...
    const userdetailsTableColumns=[
        ...
        { Header: 'Quantity', accessor: 'PickQuantity', modalType: 'textbox' },
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/ProductivityMove/GetProductivityMoveReport', body).then(
            res => {
                let data = res.data;
    ...
    const userSummaryTableColumns = [
        ...
        { Header: 'Moves', accessor: 'NoOfMove' },
        { Header: 'Moves/Hr', accessor: 'Units_Hr' },
    ...
    const fetchtransactionButtonHandler = async (data)=>
    {
        ...
        setTableLoading(true);
        await api.post('/Maintenance/TransactionHistory/GetAllSelectedUserTransactionProdMove', body).then(
            res => {
    ...
    for (var k = 0; k < rows.length; k++)  
    {
        ...
        Lines = parseInt(Lines) + parseInt(rows[k]["NoOfMove"]);
    ...
    const OpenReportPrintTab = () => {
            ...
            history.push('/Report/ProductivityMovePrint');
            // if (win) {
    ...
    const ProductivityMovePrintButton = () => {
        return (
            <React.Fragment> 
        ...
        <div>
            <TableScreen
                table={<Table data={tableData} tableColumns={tableColumns} title={tableTitle} isLoading={tableLoading}
                    AdditionalButtons={ProductivityMovePrintButton} conditionalAction ={conditionalActions}
                ></Table>}
                modal={
                    <React.Fragment>
