...
const ProductivityRepReport = () => {
    ...
    const [ReplenType, setReplenType] = useState([]);
    ...
    const tableColumns = [
        ...
        {Header: 'Rep Type', accessor: 'RepType', Filter: SelectMultipleFilter, filter: 'contains', modalType: 'dropdown',  FilterValue :ReplenType, SetFilterValue : setReplenType},
    ...
    const userdetailsTableColumns=[
        ...
        { Header: 'Rep Type', accessor: 'RepType', modalType: 'textbox'  },
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/Productivity/GetProductivityRepReport_Asics', body).then(
            res => {
                let data = res.data;
    ...
    const userSummaryTableColumns = [
        ...
        { Header: 'Lines/Hr', accessor: 'Units_Hr' },
    ...
    const fetchtransactionButtonHandler = async (data)=>
    {
        ...
        setTableLoading(true);
        await api.post('/Report/Productivity/GetAllSelectedUserTransactionProdRep_Asics', body).then(
            res => {
    ...
    filteredRows = [];
    if  (ReplenType.length > 0)
    {
        for (var j =0 ; j < rows.length; j ++)
        {
            for (var k = 0; k < ReplenType.length; k++)
              {
                  if (ReplenType[k] === rows[j]["RepType"])
                  {
                    filteredRows.push(rows[j]);
                  }
