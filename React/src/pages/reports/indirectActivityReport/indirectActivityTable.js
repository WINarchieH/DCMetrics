...
const tableTitle = 'Indirect Activity Report';

const IndirectActivityReport = () => {
    ...
    const tableColumns = [
        ...
        { Header: 'Task', accessor: 'TaskName', Filter: SelectMultipleFilter, filter: 'contains', modalType: 'dropdown', FilterValue: task , SetFilterValue: settask },
        ...
        { Header: 'Down Time', accessor: 'DownTime', modalType: 'textbox'},
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/IndirectActivityReport/GetIndirectActivityReport', body).then(
            res => {
                let data = res.data;
    ...
    const OpenReportPrintTab = () => {
            history.push('/Report/IndirectActivityPrint');
            // if (win) {
        ...
        <div>
            <TableScreen
                table={<Table data={tableData} tableColumns={tableColumns} title={tableTitle} isLoading={tableLoading}
                    AdditionalButtons={IndirectActReportButton}
                ></Table>}
...
export default IndirectActivityReport;
