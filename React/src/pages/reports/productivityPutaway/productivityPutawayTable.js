...
const tableTitle = 'Productivity Putaway Report';

const ProductivityPutawayReport = () => {
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/ProductivityPutaway/GetProductivityPutawayReport', body).then(
            res => {
                let data = res.data;
    ...
    const userSummaryTableColumns = [
        ...
        { Header: 'Putaways', accessor: 'NoOfPutaway' },
        { Header: 'Putaways/Hr', accessor: 'Units_Hr' },
    ...
    const fetchtransactionButtonHandler = async (data)=>
    {
        ...
        setTableLoading(true);
        await api.post('/Maintenance/TransactionHistory/GetAllSelectedUserTransactionProdPutaways', body).then(
            res => {
    ...
    const OpenReportPrintTab = () => {
            ...
            history.push('/Report/ProductivityPutawayPrint');
            // if (win) {
    ...
    for (var k = 0; k < rows.length; k++)  
    {
        ...
        Lines = parseInt(Lines) + parseInt(rows[k]["NoOfPutaway"]);
    ...
    const ProductivityPutawayPrintButton = () => {
        return (
            <React.Fragment>
        ...
        <div>
            <TableScreen
                table={<Table data={tableData} tableColumns={tableColumns} title={tableTitle} isLoading={tableLoading}
                    AdditionalButtons={ProductivityPutawayPrintButton} conditionalAction ={conditionalActions}
                ></Table>}
                modal={
                    <React.Fragment>
...
export default ProductivityPutawayReport;
