...
const tableTitle = 'Productivity Pack Report';

const ProductivityPackReport = () => {
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/ProductivityPack/GetProductivityPackReport', body).then(
            res => {
                let data = res.data;
    ...
    const userSummaryTableColumns = [
        ...
        { Header: 'Lines', accessor: 'NoOfPack' },
    ...
    const fetchtransactionButtonHandler = async (data)=>
    {
        ...
        setTableLoading(true);
        await api.post('/Maintenance/TransactionHistory/GetAllSelectedUserTransactionProdPack', body).then(
            res => {
    ...
    for (var k = 0; k < rows.length; k++)  
    {
        ...
        Lines = parseInt(Lines) + parseInt(rows[k]["NoOfPack"]);
    ...
    const OpenReportPrintTab = () => {
            ...
            history.push('/Report/ProductivityPackPrint');
            // if (win) {
        ...
        <div>
            <TableScreen
                table={<Table data={tableData} tableColumns={tableColumns} title={tableTitle} isLoading={tableLoading}
                    AdditionalButtons={ProductivityPackPrintButton}  conditionalAction={conditionalActions} 
                ></Table>}
                modal={
                    <React.Fragment>
...
export default ProductivityPackReport;
