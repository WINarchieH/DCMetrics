...
const tableTitle = ' User Detail Report';
...
const SummaryReport = () => {
    ...
    const tableColumns = [
        ...
        { Header: 'Start Time', accessor: 'TaskStart', modalType: 'textbox' },
        { Header: 'End Time', accessor: 'TaskEnd', modalType: 'textbox' },
        { Header: 'Lost Time', accessor: 'LostTime', modalType :'textbox' },
        { Header: 'Actual Time', accessor: 'ActualTime', modalType: 'textbox' },
        { Header: 'No Of Lines', accessor: 'NoOfLines', modalType: 'textbox' },
        { Header: 'Units Per Hour', accessor: 'UnitsPerHour', modalType :'textbox' },
        { Header: 'Break', accessor: 'Break', modalType :'textbox' },
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/SummaryReport/GetUserSummaryReport', body).then(
            res => {
                let data = res.data;
            ...
            //    if (parseFloat(x.ActualTime))
            //    {
            //        x.ActualTime = (x.ActualTime).toFixed(2);
            //    }
                    return x;
    ...
    const OpenReportPrintTab = () => {
            ...
            history.push('/Report/UserSummaryReportPrint');
            // if (win) {
        ...
        <div>
            <TableScreen
                table={<Table data={tableData} tableColumns={tableColumns} title={tableTitle} isLoading={tableLoading}
                    AdditionalButtons={UserSummaryReportButton}
                ></Table>}
