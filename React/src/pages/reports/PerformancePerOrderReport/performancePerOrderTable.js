...
const tableTitle = 'Performance Per Order Report';

const PerformancePerOrderReport = () => {
    ...
    const tableColumns = [        
        ...
        { Header: 'Employee Name', accessor: 'Employee', modalType: 'textbox' },        
    ...
    const getTable = async (fromDate, toDate) => { // TODO: API Request for table
        ...
        await api.post('/Report/PerformancePerOrder/GetPerformanceReportPerOrder', body).then(
            res => {
                let data = res.data;
        ...
        <div>
            <TableScreen
                table={<Table data={tableData} tableColumns={tableColumns} title={tableTitle} isLoading={tableLoading}
                    AdditionalButtons={PerformanceButton}
                ></Table>} />
        </div>
    )
};

export default PerformancePerOrderReport;
